...
const agriFormSubmissionStatsData = [
  { name: 'Fert.Lic.', submitted: 45, approved: 30, rejected: 5 }, // Shortened name
  { name: 'Form A1', submitted: 60, approved: 55, rejected: 2 },
  { name: 'Insp(F)', submitted: 25, approved: 20, rejected: 1 }, // Shortened name
  { name: 'Form V', submitted: 30, approved: 22, rejected: 3 },
  { name: 'Form IV', submitted: 18, approved: 15, rejected: 0 },
  { name: 'Insp(I)', submitted: 22, approved: 19, rejected: 1 }, // Shortened name
];
...
const DashboardModule = () => {
  const { seizures, labSamples, firCases, inspectionTasks, setActiveTab } = useAppContext();
  const router = useRouter();
  const [dynamicBarSize, setDynamicBarSize] = useState(15); // Default for server/initial render
  ...
  const [dynamicVerticalBarSize, setDynamicVerticalBarSize] = useState(12); // Default for server/initial render
  ...
  const hotspotData = [
   ...
   const hotspotChartConfig = {
    riskLevel: { label: "Risk %", color: "hsl(var(--chart-1))" }, // Shortened label
    violations: { label: "Violations", color: "hsl(var(--chart-2))" },
  }
    ...
    <div className="space-y-4 sm:space-y-6">
        ...
        <div className="flex items-center">
          ...
          <div>
            <h3 className="font-semibold text-destructive text-sm sm:text-base">
              Escalation Alert: {firCases.filter(f => f.status !== 'closed').length} Pending FIRs
            </h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        <StatCard
          ...
          value={inspectionTasks.length.toString()}
          trend="+12%"
          icon={<Calendar />}
          color="blue"
        />
        <StatCard
          ...
          value={seizures.filter(s => s.status !== 'closed').length.toString()}
          trend="+8%"
          color="yellow"
          icon={<Package />}
        />
        <StatCard
          title="Lab Samples"
          value={labSamples.filter(ls => ls.status !== 'completed').length.toString()}
          trend="5 pending"
          color="purple"
          icon={<Building />}
        />
        <StatCard
          title="FIR Cases"
          value={firCases.filter(f => f.status !== 'closed').length.toString()}
          trend="3 new"
          color="red"
          icon={<Scale />}
        />
                ...
                <ChartContainer config={agriFormChartConfig} className="w-full h-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={agriFormSubmissionStatsData} margin={{ top: 5, right: 5, left: -30, bottom: 5 }}>
                      ...
                      <XAxis dataKey="name" tickLine={false} axisLine={false} tickMargin={8} angle={-30} textAnchor="end" height={70} interval={0} fontSize={9} smFontSize={10}/>
                      <YAxis tickLine={false} axisLine={false} tickMargin={8} fontSize={9} smFontSize={10}/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 sm:space-y-3">
            {officerPerformance.slice(0, 4).map((officer, index) => ( // Show fewer on smaller screens or use scroll
      ...
      <Card>
        ...
        <CardContent className="h-[250px] sm:h-[300px] p-2">
          <ChartContainer config={hotspotChartConfig} className="w-full h-full">
            <BarChart data={hotspotData} layout="vertical" margin={{ left: 0, right: 10, top: 5, bottom: 5 }}> {/* Adjusted left margin */}
              <CartesianGrid strokeDasharray="3 3" horizontal={false} />
              <XAxis type="number" tickLine={false} axisLine={false} tickMargin={8} fontSize={10} smFontSize={12}/>
              <YAxis dataKey="area" type="category" tickLine={false} axisLine={false} tickMargin={8} width={60} smWidth={80} fontSize={10} smFontSize={12}/>
              <ChartTooltip content={<ChartTooltipContent indicator="dot" />} />
