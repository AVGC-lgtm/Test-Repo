...
export function AppHeader() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-2 sm:gap-4 border-b bg-background px-3 sm:px-4 md:static md:h-auto md:border-0 md:bg-transparent md:px-6 py-3 sm:py-4">
       <SidebarTrigger className="md:hidden" /> {/* Mobile sidebar toggle for main sidebar */}
      ...
      <div className="flex items-center space-x-1.5 sm:space-x-3 md:space-x-4">
        <Select value={userRole} onValueChange={setUserRole}>
          <SelectTrigger className="w-[140px] sm:w-[180px] h-8 sm:h-9 text-xs sm:text-sm" aria-label="Select user role">
            <SelectValue placeholder="Select role" />
          </SelectTrigger>
          <SelectContent>
            {Object.values(USER_ROLES).map(role => (
              <SelectItem key={role} value={role} className="text-xs sm:text-sm">{role}</SelectItem>
            ))}
          </SelectContent>
        </Select>
        ...
        >
          {darkMode ? <Sun size={16} smSize={18} /> : <Moon size={16} smSize={18} />}
        </Button>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-64 sm:w-80">
            <DropdownMenuItem className="text-xs sm:text-sm">Notification 1: High alert!</DropdownMenuItem>
            <DropdownMenuItem className="text-xs sm:text-sm">Notification 2: Task assigned.</DropdownMenuItem>
            <DropdownMenuItem className="text-xs sm:text-sm">Notification 3: Report ready.</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="relative h-8 sm:h-9 w-auto px-1.5 sm:px-2 gap-1 sm:gap-2">
              <Avatar className="h-6 w-6 sm:h-7 sm:w-7">
                <AvatarImage src={`https://placehold.co/40x40.png?text=${userInitials}`} alt={userRole} data-ai-hint="avatar person" />
