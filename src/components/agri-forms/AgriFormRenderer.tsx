...
const AgriFormRenderer: React.FC<AgriFormRendererProps> = ({ formType }) => {
  ...
  const inspectionFields = [
    { name: "dealerName", label: "Dealer/Manufacturer Name", required: true },
    { name: "registrationNumber", label: "Certificate of Registration Number", required: true },
    { name: "samplingDate", label: "Date of Sampling", type: "date", required: true },
    { name: "fertilizerTypeGrade", label: "Type and Grade of Fertilizer", required: true },
    { name: "batchNoManufactureDate", label: "Batch Number and Date of Manufacture", required: true },
    { name: "stockPosition", label: "Stock Position", required: true },
    { name: "specificationFCO", label: "Specification as per FCO", required: true },
    { name: "compositionAnalysis", label: "Composition as per Analysis", required: true },
    { name: "moisture", label: "Moisture (%)", required: false },
    { name: "totalN", label: "Total N (%)", required: false },
    { name: "totalP2O5", label: "Total P2O5 (%)", required: false },
    { name: "totalK2O", label: "Total K2O (%)", required: false },
    { name: "remarks", label: "Remarks", type: "textarea", required: false },
  ];

  const InspectionForm: React.FC<{ inspectionId: string }> = ({ inspectionId }) => {
    const [form, setForm] = useState<Record<string, string>>({});
    const [loading, setLoading] = useState(false);
    const [submitted, setSubmitted] = useState(false);

    useEffect(() => {
      // Auto-mark inspection as seen
      axios.put(`/api/inspections/${inspectionId}/seen`).catch(() => {});
    }, [inspectionId]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      setLoading(true);
      try {
        await axios.post(`/api/inspections/${inspectionId}/data`, form);
        setSubmitted(true);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle>Scheduled Inspection Data Entry</CardTitle>
        </CardHeader>
        <CardContent>
          {submitted ? (
            <div className="text-green-600 font-semibold">Inspection data submitted successfully.</div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-3">
              {inspectionFields.map(field => (
                <div key={field.name}>
                  ...
                  {field.type === "textarea" ? (
                    <Textarea id={field.name} name={field.name} value={form[field.name] || ""} onChange={handleChange} />
                  ) : (
                    <Input
                      id={field.name}
                      name={field.name}
              ...
              <Button type="submit" disabled={loading}>{loading ? "Saving..." : "Submit Inspection"}</Button>
            </form>
          )}
        </CardContent>
      </Card>
  ...
  return (
    <div>
      ...
      {formType === 'scheduledInspection' && <InspectionForm inspectionId="12345" />} {/* Replace with actual ID or prop */}
    </div>
  );
};
